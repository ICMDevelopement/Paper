...
if [ ! -f  "$jarpath.jar" ]; then
    ...
    curl -s -o "$jarpath.jar" "http://www.mc-download.com/index.php?action=downloadfile&filename=minecraft-server-1.8.8.jar&directory=Minecraft%20Versions%20Official/Minecraft%20Server&"
    if [ "$?" != "0" ]; then
